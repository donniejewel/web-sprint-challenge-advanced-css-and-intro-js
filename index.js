...
//*** */ let years=parsInt(data[i].years)
...
let arr=[];
for (let i=0; i < data.length; i++){
  let years= parseInt(data[i].years)
  if (years >=1900 && years <=2000) {
...
array.slice(array, array[index])   
}

console.log(removeArtist(artists, 4));
console.log(artists[4].name); 
...
let donnie={id, name, years, genre, nationality, bio};  
artists.push(donnie);
}

console.log(addArtist(21, "Donnie H-H", 1995, "Web Design", "Undefined", "A thing about a thing.."));
console.log(artists);
